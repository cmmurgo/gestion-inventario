...
const EditarVenta = () => {
  ...
  const verDetalle = (detalle) => {  
    navigate(`/ventas/${id}/detalle/${detalle.id}/ver`);
  };

  const editarDetalle = (detalle) => {
    navigate(`/ventas/${id}/detalle/${detalle.id}/editar`);
  };
  ...
  const eliminarDetalle = async (detalleId) => {
    if (window.confirm('¿Eliminar este detalle?')) {
      const token = localStorage.getItem('token');
      try {
        await axios.delete(`${API_URL}/api/ventas/${id}/detalle/${detalleId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        cargarDetalles(token);
      } catch (error) {
        ...
        <tbody>
          {currentDetallesVentas.length > 0 ? (
            currentDetallesVentas.map((detalle, index) => (
                ...
                <td>      
                    <button className="btn btn-link text-primary me-2" onClick={() => verDetalle(detalle)}><FaEye /></button>
                    <button className="btn btn-link text-warning me-2" onClick={() => editarDetalle(detalle)}><FaEdit /></button>
                    <button className="btn btn-link text-danger" onClick={() => eliminarDetalle(detalle)}><FaTrash /></button>
                </td>
              </tr>
            ))
          ) : (
